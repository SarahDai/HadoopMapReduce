...
public class Task3_3DistributedCache extends Configured implements Tool {
	...
	public static class ReduceClass extends Reducer<Text, Text, Text, Text> {
	   ...
	   //@SuppressWarnings("deprecation")
		public void setup(Context context) throws java.io.IOException,InterruptedException {
		   ...
		   CustomerFileNm = localFiles[0].toString();
		   
		   BufferedReader bf = new BufferedReader(new FileReader(CustomerFileNm)); 
		   ...
		   while((line = bf.readLine()) != null)
		   {
			   ...
			   custID = Integer.parseInt(column[0]);
			   nameStr = column[1];
			   salary = column[4];
			   
			   custIDName.put(custID, nameStr+","+salary);
		   }
		...
		//@Override
	     ...
	     public int run(String[] args) throws Exception {
			    ...
			    // VV IMP: all that is to be configured to conf should be configured before adding conf to job
			    DistributedCache.addCacheFile(new URI("/user/hadoop/cs561/dataset/Customer.txt"), conf);
			    
			    Job job = new Job(conf, "Task3_3DistributedCache");
			    job.setJarByClass(Task3_3DistributedCache.class);

			    //Path custIn = new Path(args[0]);
			    Path transIn = new Path(args[0]);
			  ...
			  public static void main(String[] args) throws Exception {
			     int res = ToolRunner.run(new Configuration(), new Task3_3DistributedCache(), args);
